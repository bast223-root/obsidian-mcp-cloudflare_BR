...
- `UPLOAD_TOKEN` — Workers secret. Bearer token for the `/upload` endpoint and the HMAC key for signed upload links. Unset disables the upload endpoint and `create_upload_link`; a value below `MIN_SECRET_LEN` (16) fails closed (503, `upload_misconfigured`). Set the same way as `AUTH_PASSWORD` (`wrangler secret put` or `secrets:push`). Augmented onto `Env` by hand in `src/types.ts` (secrets aren't in the generated `worker-configuration.d.ts`); that augmentation also extends `Cloudflare.Env` so the `cloudflare:test` env stays assignable. Augmented onto `Env` by hand in `src/types.ts` (secrets aren't in the generated `worker-configuration.d.ts`); that augmentation also extends `Cloudflare.Env` so the `cloudflare:test` env stays assignable.
- `VAULT_PREFIX`, `DAILY_NOTE_PATH_TEMPLATE`, `PERMALINK_BASE_URL`, the `ATTACHMENT*`/`ATTACHMENTS_*` vars, and `SERVICE_BASE_URL` — plain vars. `SERVICE_BASE_URL` is this Worker's own public origin, used by `create_upload_link` to build absolute links; defaults to `https://${MCP_HOSTNAME}` in the template. The env→`VaultConfig` mapping lives in `src/config.ts` (`buildVaultConfig`), shared by the MCP agent and the upload handler so both apply identical attachment policy.
