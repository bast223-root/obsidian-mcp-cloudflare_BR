...
- `UPLOAD_TOKEN` — Workers secret. Bearer token for the `/upload` endpoint and the HMAC key for signed upload links. Unset disables the upload endpoint and `create_upload_link`. Augmented onto `Env` by hand in `src/types.ts` (secrets aren't in the generated `worker-configuration.d.ts`); that augmentation also extends `Cloudflare.Env` so the `cloudflare:test` env stays assignable.
