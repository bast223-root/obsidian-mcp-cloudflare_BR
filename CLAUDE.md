...
- `AUTH_PASSWORD` — Workers secret (set/rotated via `npx wrangler secret put AUTH_PASSWORD`, or `npm run secrets:push` which resolves a value/`op://` reference from the gitignored `.secrets.env`).
- `UPLOAD_TOKEN` — Workers secret. Bearer token for the `/upload` endpoint and the HMAC key for signed upload links. Unset disables the upload endpoint and `create_upload_link`. Set the same way as `AUTH_PASSWORD` (`wrangler secret put` or `secrets:push`). Augmented onto `Env` by hand in `src/types.ts` (secrets aren't in the generated `worker-configuration.d.ts`); that augmentation also extends `Cloudflare.Env` so the `cloudflare:test` env stays assignable.
