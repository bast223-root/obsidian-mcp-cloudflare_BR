...
- **Consent page hardening.** The OAuth consent page HTML-escapes all interpolated values (client name, error text) and is served with `Content-Security-Policy: default-src 'none'`, `X-Frame-Options: DENY`, and `Referrer-Policy: no-referrer`. It is CSRF-protected with a double-submit token (an `HttpOnly; Secure; SameSite=Strict` cookie plus a matching hidden field; mismatches are rejected with HTTP 403). The round-tripped `oauthReqInfo` state is HMAC-signed (keyed by `AUTH_PASSWORD`) so a tampered blob is rejected before any field in it is trusted.
