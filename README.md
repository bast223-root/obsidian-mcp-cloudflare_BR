...
| `move_attachment(from_path, to_path, overwrite?, update_embeds?)` | Move/rename an attachment server-side (R2 copy+delete — works for large files). Both paths allowlisted. By default rewrites the embed in **every** note that referenced the old path (links follow the file across one or many notes); `update_embeds: false` moves bytes only. Ordering is copy → rewrite → **delete-old-last**, so the only irreversible step runs after the rewrites succeed; if a rewrite fails the move rolls back (best-effort — R2 has no cross-object transaction) and returns `embed_rewrite_failed` with nothing committed, rather than stranding a half-move. Returns JSON `{from, to, embed_markdown, etag, size, content_type, notes_modified, referrers_unchanged}` — `notes_modified` are the rewritten notes; `referrers_unchanged` are notes that link the file by bare filename (`![[name.ext]]`), left as-is because Obsidian resolves those by name regardless of folder. Failures: `same_path`, `not_found`, `exists`, `disallowed_extension`, `embed_rewrite_failed` |
...
- **Transport.** The `/authorize`, `/upload`, and `/health` routes redirect plaintext HTTP to HTTPS (308). Enable Cloudflare **"Always Use HTTPS"** at the zone for full coverage — it also protects `/mcp` and `/token`, which the OAuth library handles directly.
- **Consent page hardening.** The OAuth consent page HTML-escapes all interpolated values (client name, error text) and is served with a strict `Content-Security-Policy` (`default-src 'none'`; scripts limited to Cloudflare's auto-injected analytics beacon; inline style only), `X-Frame-Options: DENY`, `base-uri 'none'`, and `Referrer-Policy: no-referrer`. (No `form-action` directive: the OAuth flow legitimately 302s the form submission to the client's callback origin, which browsers enforce `form-action` against.) It is CSRF-protected with a double-submit token (an `HttpOnly; Secure; SameSite=Strict` cookie plus a matching hidden field; mismatches are rejected with HTTP 403). The round-tripped `oauthReqInfo` state is HMAC-signed (keyed by `AUTH_PASSWORD`) so a tampered blob is rejected before any field in it is trusted.
...
- **SSRF surface (`upload_attachment_url`).** The server-side URL fetch is **default-closed**: with `ATTACHMENT_FETCH_HOST_ALLOWLIST` empty it fetches from no host. When you populate it, **list only specific, trusted hostnames** — a broad entry lets prompt injection steer the Worker to fetch attacker-chosen URLs. Setting it to the single value `*` allows any host (an explicit opt-in for pulling from arbitrary public links) — convenient, but it removes the allowlist boundary, so use it only on a deployment you alone drive. A private-IP/localhost denylist backstops the allowlist *and* `*`, and the host is re-validated on every redirect hop, but with a finite list the allowlist is the real boundary: keep it tight.
...
| `ATTACHMENT_FETCH_HOST_ALLOWLIST` | *(empty)* | CSV of hostnames `upload_attachment_url` may fetch from. **Default-closed:** empty ⇒ the tool fetches from no host (`host_not_allowed`). The single value `*` allows any host (explicit opt-in; the SSRF denylist still applies). Deployment-specific, so it's filled from `.env` (not baked into `wrangler.example.jsonc`); rerun `npm run setup` after changing it. |
